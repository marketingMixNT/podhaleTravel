...
<div class="flex flex-col group gap-2">
 ...
 {{-- IMG & CATEGORIES --}}
        <div class="overflow-hidden relative">
            ...
            <div class="overflow-hidden">
                <img src="{{ $attraction->getThumbnailUrl() }}"
                    alt="zdjęcie przedstawiające podhalańską atrakcję - {{ $attraction->name }}" class="aspect-wideo object-cover h-[400px] scale-hover w-full" width="635" height="400" loading="lazy">
            </div>
            {{-- categories --}}
            <div class="flex justify-start items-center mt-4  gap-y-3 flex-wrap">
                ...
                @foreach ($attraction->categories as $category)
                    <x-base.badge class="mr-4" 
                        wire:navigate href="{{ route('attraction.index', ['category' => $category->slug]) }}">{{ $category->getFormatName() }}</x-base.badge>
                @endforeach
            </div>
        </div>
        {{-- CONTENT --}}
        <div class="flex justify-between items-center px-2 py-4">
            ...
            <div class="self-end mb-3"><x-base.link-btn href="{{ route('attraction.show', $attraction->slug) }}">Sprawdź</x-base.link-btn></div>
        </div>
    </a>
</div>
